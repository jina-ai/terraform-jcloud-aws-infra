...
module "prometheus_stack" {
  ...
  grafana_database_type                       = var.grafana_database_type
...
module "loki" {
  source                          = "./modules/loki"
  count                           = (var.enable_logging || var.enable_loki) ? 1 : 0
  namespace                       = local.monitor_namespace
  ...
  depends_on                      = [module.monitor_store]
}
...
module "thanos" {
  ...
  count                             = (var.enable_metrics || var.enable_thanos) ? 1 : 0
  namespace                         = local.monitor_namespace
  thanos_object_storage_config_name = var.thanos_object_storage_config_name
  thanos_object_storage_config_key  = var.thanos_object_storage_config_key
